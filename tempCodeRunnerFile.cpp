...
int handleCount(const string &sym_type)
{
	int count, op_count = 1, id_count = 1, keyword_count = 1, lit_count = 1, relop_count = 1, delimiter_count = 1;
	if (sym_type == "keyword")
		count = keyword_count++;
	else if (sym_type == "op")
		count = op_count++;
	...
	for (int i = 0; i < n; i++)
	{
		...
		else if (isalpha(string_input[i])) // an identifer or keyword
		{
			...
			do
			{
				...
				k++;
				next = i + 1;
				if (isalpha(string_input[next]) || (string_input[next] == '.' && isalpha(string_input[next + 1])))
				{
					next_is_alphabet = 1;
					i++;
				}
